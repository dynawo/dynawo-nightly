...
if [ "$TRAVIS_OS_NAME" = "linux" ]; then
  ...
  curl -v -L -H "Authorization: token $GITHUB_TOKEN" $(curl -s -L -H "Authorization: token $GITHUB_TOKEN" -X GET https://api.github.com/repos/dynawo/dynawo/releases/latest | grep "Dynawo_MacOS" | grep url | cut -d '"' -f 4) -o $HOME/Dynawo_MacOS_latest.zip
